...
class Covariates:
    ...
    def prepare_earn_series(self, tickers=None):
        ...
        earn_df = self.earnings_loaded_df
    ...
    def prepare_key_metrics(self, stock_price_series=None):
        kms_loaded_df = self.kms_loaded_df
        # print(kms_loaded_df)
    ...
    def load_past_covariates(self):
        self.earnings_loaded_df = self.load_earnings()
        self.kms_loaded_df = self.load_key_metrics()

    def load_future_covariates(self):
        self.est_quarter_loaded_df = self.load_estimates(period="quarter")
        self.est_annual_loaded_df = self.load_estimates(period="annual")
    ...
    def load_earnings(self):
        ...
        earnings_loaded_df["date"] = pd.to_datetime(self.earnings_loaded_df["date"])
        earnings_unique = self.earnings_loaded_df.drop_duplicates(
            subset=["symbol", "date"]
        )
    ...
    def load_key_metrics(self):
        ...
        return kms_loaded_df

    def load_estimates(self, period: str = None):
    ...
    def prepare_analyst_estimates(self, tickers=None):
        ...
        annual_loaded_df = self.est_annual_loaded_df
        annual_est_series = self.prepare_est_series(
            all_est_df=annual_loaded_df,
            n_future_periods=2,
            period="annual",
            tickers=tickers,
        )
    ...
    def prepare_past_covariates(
        ...
        print(
            f"past covariates with key metrics sample: \n{past_covariates[t][0:5].pd_dataframe()}"
        )
        # Add broad market indicators to past covariates
        broad_market_series = self.prepare_broad_market_indicators(
            csv_file="data/broad_market.csv.bz2", train_date_start=train_date_start
        )
    ...
    def __add_holidays(self, t_series: dict = None):
        new_series = {}
        for t, series in t_series.items():
    ...
    def prepare_future_covariates(self, ticker_series=None, min_samples=None):
        # add analyst estimates
        quarter_est_series, annual_est_series = self.prepare_analyst_estimates(
            csv_file_quarter="data/analyst_estimates_quarter.csv.bz2",
            csv_file_annual="data/analyst_estimates_annual.csv.bz2",
            tickers=ticker_series.keys(),
        )
        ## Stack analyst estimates to future covariates
        stacked_future_covariates = self.stack_covariates(
            old_covs=future_covariates, new_covs=quarter_est_series
        )
        stacked_future_covariates = self.stack_covariates(
            old_covs=stacked_future_covariates,
